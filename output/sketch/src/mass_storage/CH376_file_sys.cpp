...
UINT8	Wait376Interrupt( void )  /* 等待CH376中断(INT#低电平)，返回中断状态码, 超时则返回ERR_USB_UNKNOWN */
{
    ...
    //#endif
    CH376DebugOut("Wait376Interrupt Done");
	...
	for ( i = 0; i < 5000000; i ++ ) {  /* 计数防止超时,默认的超时时间,与单片机主频有关 */
		if ( Query376Interrupt( ) ) {  /* 检测到中断 */
      Serial3.read();  // 先清除接收缓冲区再去获取中断状态码
      return( CH376GetIntStatus( ) ); 
    }
    /* 在等待CH376中断的过程中,可以做些需要及时处理的其它事情 */
	}

  CH376DebugOut("Wait376Interrupt timeout");
	return( ERR_USB_UNKNOWN );  /* 不应该发生的情况 */
#endif
}
#endif
...
{
  CH376DebugOut("CH376SendCmdWaitInt");
...
{
  CH376DebugOut("DiskConnect");

	if ( Query376Interrupt( ) ) {
	  CH376GetIntStatus( );  /* 检测到中断 */
	  CH376DebugOut("GetIntStatus");
	}
...
// 此函数只打开路径至路径截至符 ，如 /a/b/c 如果指针在 a 则只打开 a
...
UINT8	CH376ByteReadOne( PUINT8 buf, PUINT16 RealCount )  /* 以字节为单位从当前位置读取数据块 */
{
	UINT8	s;
	xWriteCH376Cmd( CMD2H_BYTE_READ );
	xWriteCH376Data( (UINT8)1 );
	xWriteCH376Data( (UINT8)(1>>8) );
	xEndCH376Cmd( );
	if ( RealCount ) *RealCount = 0;
	while ( 1 ) {
		s = Wait376Interrupt( );
